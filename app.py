...
def process():
    ...
    if request.method == 'POST':
        selected_features = principal_features  # Use all principal features for auto clustering
        scatter_plot, radar_chart, clustered_data = process_data(selected_features)
        return render_template('process.html', scatter_plot=scatter_plot, radar_chart=radar_chart, clustered_data=clustered_data.to_html(), selected_features=selected_features)
